...
function createPNG(size) {
  ...
  // Mic dimensions relative to size
  const cx = w / 2;
  const micW = size * 0.2;
  ...
  const micRx = micW / 2;
  const arcR = size * 0.2;
  const arcCy = size * 0.48;
  const arcStroke = size * 0.05;
  const stickTop = size * 0.68;
  const stickBot = size * 0.78;
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      if (x >= micLeft && x <= micRight) {
        if (y >= bodyTop && y <= bodyBot) isMic = true;
        // Top cap
        const dtx = x - cx, dty = y - bodyTop;
        if (dty < 0 && (dtx * dtx + dty * dty) <= micRx * micRx) isMic = true;
        // Bottom cap
        const dby = y - bodyBot;
        if (dby > 0 && (dtx * dtx + dby * dby) <= micRx * micRx) isMic = true;
      }

      // Arc (half circle below mic)
      const dx = x - cx, dy = y - arcCy;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dy >= 0 && dist >= arcR - arcStroke / 2 && dist <= arcR + arcStroke / 2) isMic = true;

      // Stick
      if (Math.abs(x - cx) <= arcStroke / 2 && y >= stickTop && y <= stickBot) isMic = true;

      // Base
      if (Math.abs(y - stickBot) <= arcStroke / 2 && x >= cx - baseW && x <= cx + baseW) isMic = true;

      raw[px] = isMic ? 26 : 255;
      raw[px + 1] = isMic ? 26 : 255;
      raw[px + 2] = isMic ? 26 : 255;
      raw[px + 3] = 255;
    }
  }
